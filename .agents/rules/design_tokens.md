...
4. **Polices personnalisées et Graisses (Font Family vs Font Weight)** : Dans vos styles React Native (StyleSheet), n'utilisez pas la propriété `fontWeight` avec les polices de caractères personnalisées. Vous devez spécifier directement le nom de la police incluant sa variante (ex. `Satoshi-Medium`, `Satoshi-Bold`, `BricolageGrotesque-SemiBold` dans la propriété `fontFamily`) car le moteur de rendu de React Native / Expo ne résout pas automatiquement les graisses sur toutes les plateformes (notamment Android).
